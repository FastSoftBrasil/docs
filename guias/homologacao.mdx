...
Pré-requisito: o(s) domínio(s) onde o botão aparece devem estar **registrados e verificados** via API (`POST /v1/apple-pay/domains` → hospedar o arquivo → `validate`), conforme o [guia de Apple Pay](/guias/apple-pay) — não é necessário conta Apple Developer nem certificados. A cobrança Apple Pay é enviada como cartão de crédito acompanhada do objeto de carteira (`wallet` com o token do Apple Pay), sempre em `BRL` e com `customer.document` presente.
...
| AP-03 | **Domínio registrado e verificado** (`GET /v1/apple-pay/domains`) | Domínio com `status: "verified"`; sessão validada pelo `POST /v1/apple-pay/validate-merchant` sem erro | Resposta do GET mostrando `verified` |
