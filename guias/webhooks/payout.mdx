...
| `amount` | number | Valor bruto solicitado (em reais) |
| `currency` | string | Código ISO da moeda (ex: `BRL`) |
| `status` | string | Status atual (ver tabela acima) |
| `payoutFee` | number | Taxa de saque calculada (em reais) |
| `netAmount` | number | Valor líquido após dedução das taxas (em reais) |
...
app.post('/webhooks/fastpay', (req, res) => {
  ...
  switch (event) {
    case 'payout.approved':
      ...
      console.log(`Valor líquido: R$ ${data.netAmount.toFixed(2)}`);
      console.log(`Taxa: R$ ${data.payoutFee.toFixed(2)}`);
...
```javascript
const amountInReais = data.amount;       // já em reais
const feeInReais = data.payoutFee;       // já em reais
const netAmountInReais = data.netAmount; // já em reais
```
